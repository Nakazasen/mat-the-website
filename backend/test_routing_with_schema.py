...
try:
    from main import get_provider_router, resolve_ai_provider_config, AIRequest, build_multilocale_object_schema
except ImportError:
    from backend.main import get_provider_router, resolve_ai_provider_config, AIRequest, build_multilocale_object_schema
...
async def test_translation_routing():
    ...
    # Try a simple translation request with schema
    user_prompt = "Hãy dịch câu này sang các ngôn ngữ: 'Tôi muốn sinh tồn trong mạt thế.'"
    system_instruction = "Bạn là biên dịch viên chuyên nghiệp. Bạn phải trả về một JSON object phù hợp với schema được cung cấp."
    ...
    print("\n⏳ Routing translation request with SCHEMA...")
